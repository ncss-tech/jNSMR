...
if (length(bilfile) == 0) {
  ...
  # download by month (suppress txtProgressBar by capturing output)
  capture.output({
    res <- lapply(1:12, .download_month, resolution = RESOLUTION)
  })
  # get the monthly files only (hardcoded for 30yr normals for now)
  bilfile <- list.files(PRISM_PATH, "\\.txt$|\\.bil$", recursive = TRUE)
}
```

```{r}
# capture file name components
monthly.pattern <- ".*PRISM_([a-z]+)_(30yr_normal)_(\\d+[a-z]+)M[23]_(\\d{2})_bil\\.(bil|txt)"
...
#### Moisture regimes (focus on Xeric, Aridic, Ustic, Udic)
